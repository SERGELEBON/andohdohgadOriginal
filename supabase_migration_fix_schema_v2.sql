-- ===============================================
-- MIGRATION ROBUSTE : Aligner le Schéma Supabase avec le Code Frontend
-- Version 2 (Safe) - Date : 2026-07-16
-- ===============================================
-- Cette version vérifie l'existence des colonnes avant la migration
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Migrer depuis 'name' vers 'first_name' + 'last_name'
  ...
  -- Vérifier si la colonne 'preferred_date' existe
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'appointments'
    ...
    -- Migrer depuis 'preferred_date' vers 'date' + 'time_slot'
    UPDATE public.appointments SET
...
-- Migration depuis doc_id si la colonne existe
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    -- Essayer de récupérer depuis la table documentation
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'documentation') THEN
      UPDATE public.documentation_purchases dp
      SET
        document_id = COALESCE(dp.document_id, d.id::TEXT),
        document_name = COALESCE(dp.document_name, d.title_fr, d.title, 'Document'),
        price = COALESCE(dp.price, dp.amount_paid, 0)
      FROM public.documentation d
      WHERE dp.doc_id = d.id
        AND (dp.document_id IS NULL OR dp.document_name IS NULL OR dp.price IS NULL);
    END IF;

    -- Remplir les valeurs par défaut pour les lignes restantes
  ...
  -- Si doc_id n'existe pas, remplir avec des valeurs par défaut
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'documentation_purchases'
...
-- Fonction qui crée automatiquement le profil
...
-- Créer le trigger (supprimer d'abord si existe)
